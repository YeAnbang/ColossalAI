...
def disable_mask_supervised_tokenize_sft(Args, **kwargs):
    """Disable mask for supervised tokenize sft"""
    out = supervised_tokenize_sft(Args, **kwargs)
    # out['labels'] = out['input_ids']
    # out['labels_decode'] = out['inputs_decode']
    return out
...
def main():
    ...
    parser.add_argument("--max_length", type=int, default=32*1024, help="Max length of each spliced tokenized sequence")
    ...
    if args.type=='sft':
        preparation_function = disable_mask_supervised_tokenize_sft
    ...
    dataset_statistics = defaultdict(lambda : 0)
    for index, dataset in enumerate(list_dataset):
        ...
        for data in dataset:
            dataset_statistics[int(len(data['input_ids'])/1000)] += 1
