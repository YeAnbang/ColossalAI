...
def main():
    ...
    tokenizer = LlamaTokenizer.from_pretrained(args.tokenizer_dir, use_fast=False)
    ...
    for index, dataset in enumerate(list_dataset):
        ...
        # dataset = dataset.select(random.sample([i for i in range(len(dataset))], int(len(dataset)/10)))
        print("downsample to", len(dataset))
        dataset = dataset.map(
            function=supervised_tokenize_pretrain,
            fn_kwargs={"tokenizer": tokenizer, "max_length": args.max_length,
                        "source_field":"source", "target_field":"target"},  # original prompt response/ aug source target
            keep_in_memory=False,
            num_proc=min(len(dataset), cpu_count()),
        )
        def extract_meta_source(example):
            example['meta_source'] = example['meta']['source']
            return example

        dataset = dataset.map(function=extract_meta_source, keep_in_memory=False,
            num_proc=min(len(dataset), cpu_count()))
        dataset = dataset.filter(lambda data: data["labels"] is not None,
            num_proc=min(len(dataset), cpu_count()))
        import pandas as pd
        df = dataset.to_pandas()
        distribution = df['meta_source'].value_counts(normalize=False)
        print(distribution)
        # for key in distribution:
        #     if key in overall_data_distribution:
        #         overall_data_distribution[key] += distribution[key]
        #     else:
        #         overall_data_distribution[key] = distribution[key]
        dataset = dataset.remove_columns(column_names=["source", "target", "meta", "meta_source"])
        ...
        print("ClosedToConstantLengthSplicedDataset length:", len(spliced_dataset))
