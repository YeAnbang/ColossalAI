...
def load_mixed_cont_pretrain_dataset_from_config(datasets_config: Union[Dict, str], mapping=None, random_seed=42):
    """
    Load a mixed continuous pretraining dataset from a configuration.

    Args:
        datasets_config (Union[Dict, str]): The configuration for the datasets. It can be either a dictionary or a file path to a JSON file.
    Returns:
        ConcatDataset: The concatenated dataset containing the pretraining data.
    """
    random.seed(random_seed)
    if isinstance(datasets_config, str):
        # Read dataset config
        import json
        datasets_config = json.load(open(datasets_config, "r", encoding='utf8'))
    datasets = []  # `List[datasets.dataset_dict.Dataset]`
    for name, dataset in datasets_config.items():
        data_source = dataset['tokenized_dataset_dir']
        total_sample = 0
        data_type = dataset['type']
        assert data_type in ['cpt', 'sft'], f"Unsupported data type {data_type}"
        to_keep_ratio = dataset.get('to_keep_ratio', 1.0)
        if to_keep_ratio == 0:
            continue
        for ds_path in data_source:
            ds_path = os.path.abspath(ds_path)
            assert os.path.exists(ds_path), f"Not existed file path {ds_path}"
            ds_dict = load_from_disk(dataset_path=ds_path, keep_in_memory=False)
            if isinstance(ds_dict, HFDataset):
                if to_keep_ratio!=1.0:
                    to_keep = int(to_keep_ratio * len(ds_dict))
                    sampled_dataset = ds_dict.select(random.sample(range(len(ds_dict)), to_keep))
                else:
                    sampled_dataset = ds_dict
                if mapping:
                    from multiprocessing import cpu_count
                    sampled_dataset = sampled_dataset.map(mapping, batched=False, num_proc=cpu_count())
                # sampled_dataset = sampled_dataset.add_column("sft_or_cpt", [data_type=='sft'] * len(sampled_dataset))
                total_sample += len(sampled_dataset)
                datasets.append(sampled_dataset)
            else:
                raise ValueError("Only hf dataset is allowed")
        print(f"Total sample for {name}: {total_sample}")
    if len(dataset)==0:
        raise ValueError("No data point is kept")
