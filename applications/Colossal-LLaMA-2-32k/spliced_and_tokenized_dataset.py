...
def supervised_tokenize_pretrain(
    ...
    source_field: str = "source", target_field: str = "target", category_field: str = "category"
    ...
    source_text = data_point[source_field]  # `str`
    target_text = data_point[target_field]  # `str`
    is_null_source = len(source_text) == 0
    assert len(target_text)>0, f"target_text is empty: {data_point}"
    ...
    if sequence_labels.count(ignore_index)==len(sequence_labels):
        # print(data_point['meta'])
        return dict(
            input_ids=None,
            labels=None,
            seq_length=None,
    ...
    return dict(
        ...
        seq_category=data_point[category_field] if category_field in data_point else data_point['meta']['domain'],
    )
...
class ClosedToConstantLengthSplicedDataset(IterableDataset):
    ...
    def __init__(
        ...
        category_field: str = "seq_category",
        infinite: bool = False,
        shuffle: bool = True,
        ...
        self.statistic = defaultdict(lambda: 0)
        ...
        if fetch_sequence_func is None:
            self.fetch_sequence_func = lambda data_point: (data_point[input_ids_field], data_point[labels_field], data_point[category_field])
        else:
        ...
        while more_data_points is True:
            ...
            while True:
                ...
                try:
                    ...
                    seq_input_ids, seq_labels, category = self.fetch_sequence_func(next(iterator))
                    buffer.append({self.input_ids_field: seq_input_ids, self.labels_field: seq_labels, self.category_field: category})
            ...
            for i, data_point in enumerate(buffer):
                ...
                category = data_point[self.category_field]
                # Handle special case:
                ...
                if len(seq_input_ids) > self.max_length:
                    ...
                    self.statistic[category] += self.max_len
                ...
                if len(spliced_input_ids) + len(seq_input_ids) > self.max_length:
                    ...
                    self.statistic[category]+=len(seq_input_ids)
