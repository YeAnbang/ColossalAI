...
class LlamaDynamicScaledRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=4096, base=10000, ntk=False, device=None):
        super().__init__()
        self.ntk = ntk
        self.base = base
        self.dim = dim
...
def patch_llama_for_dynamic_scaled_rotary_embeddings(model, ntk=False):
    for each in model.model.model.layers:
