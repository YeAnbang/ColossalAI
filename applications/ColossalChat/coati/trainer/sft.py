...
class SFTTrainer(SLTrainer):
    ...
    def __init__(
        ...
        self.max_number_of_checkpoints = max_number_of_checkpoints
        self.coordinator = coordinator
        self.num_train_step = 0
        self.num_eval_step = 0
        self.accumulative_meter = AccumulativeMeanMeter()
        self.few_shot_sample_prob = few_shot_sample_prob
    ...
    def _train(self, epoch: int):
        ...
        for i, batch in enumerate(self.train_dataloader):
            ...
            if self.few_shot_sample_prob:
                outputs = self.model(input_ids = batch["input_ids"], attention_mask=batch["attention_mask"], 
                                    labels=batch["labels"], few_shot_sample_prob=self.few_shot_sample_prob)
            else:
                outputs = self.model(input_ids = batch["input_ids"], attention_mask=batch["attention_mask"], 
                                    labels=batch["labels"])
            loss = outputs.loss
            # if loss is nan log the input and labels
            if is_rank_0():
                if torch.isnan(loss):
                    torch.set_printoptions(threshold=10_000)
                    print(f"Loss is nan at step {self.num_train_step}")
                    print(f"attention mask: {batch['attention_mask']}")
                    print(f"Input: {batch['input_ids']}")
                    print(f"Labels: {batch['labels']}")
                    print(torch.sum(batch['labels'] != -100, dim=-1))
                    # for i, layer in enumerate(self.model.unwrap().model.layers):
                    #     if hasattr(layer, 'self_attn'):
                    #         print("debug_attention_mask_origin")
                    #         print(layer.self_attn.debug_attention_mask_origin[:,:,2:4,:])
                    #         print(layer.self_attn.debug_attention_mask_origin.shape)
                    #         print("debug_attention_mask")
                    #         print(layer.self_attn.debug_attention_mask)
                    #         print(layer.self_attn.debug_attention_mask.shape)
            ...
            if (i + 1) % self.accumulation_steps == 0:
                ...
                self.scheduler.step()
                avg_loss = self.accumulative_meter.get("loss")
                step_bar.set_description(f"Epoch {epoch + 1}/{self.max_epochs} Loss: {avg_loss}")
                if self.writer:
                ...
                if (
                    ...
                    save_checkpoint(
                        ...
                        max_number_of_checkpoints = self.max_number_of_checkpoints
                    )
