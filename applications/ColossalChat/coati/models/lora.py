    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.r > 0:
                ...
                #     print(e)
                #     print(result.size(), self.lora_dropout(x).size(), self.lora_A.t().size(), self.lora_B.t().size())
                #     exit()
                # try:
                ...
                #     print(e)
                #     print(result.size(), self.lora_dropout(x).size(), self.lora_A.t().size(), self.lora_B.t().size())
                #     print(result.dtype, self.lora_dropout(x).dtype, self.lora_A.t().dtype, self.lora_B.t().dtype)
                #     print(result.device, self.lora_dropout(x).device, self.lora_A.t().device, self.lora_B.t().device)
                #     exit()  
...
def sanity_check_lora_require_grad(module):
    for name, child in module.named_children():
        if isinstance(child, LoraLinear):
            print("sanity_check:", name, child.lora_B.requires_grad, child.lora_A.requires_grad)
        else:
            sanity_check_lora_require_grad(child)
