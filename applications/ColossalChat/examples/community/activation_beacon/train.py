from src import get_model_and_tokenizer
import argparse
import json
import math
import os
...
from src.activation_beacon_llama.configuration_llama import LlamaConfig
from src.activation_beacon_llama.modeling_llama import LlamaForCausalLM, LlamaModel
...
def load_model(args):
    ...
    config = LlamaConfig.from_pretrained(
        ...
        beacon_stride_mix=args.beacon_stride_mix,
        beacon_attn=args.beacon_attn,
        beacon_attend_previous=args.beacon_attend_previous,
        beacon_ratio=args.beacon_ratio,
        beacon_ratio_mix=args.beacon_ratio_mix,
        beacon_param=args.beacon_param,
        retrieval_method=args.retrieval_method,
        retrieval_topk=args.retrieval_topk,
        **rope_kwargs,
    )

    # Hack: ValueError: LlamaForCausalLM does not support Flash Attention 2.0 yet.
...
def train(args):
    ...
    # )
    coordinator.print_on_master("Initializing model")
    ...
    # load original cont pretrain dataset, this dataset has 8.1B tokens, we only need 2.5B
    coordinator.print_on_master(f"start loading datasets")
    datasets = []  # `List[datasets.dataset_dict.Dataset]`
    dataset_length = 0
    for ds_path in args.augment_dataset:
    ...
    coordinator.print_on_master("augment_dataset loaded")
    coordinator.print_on_master(f"Total size: {dataset_length}")
    
    dataset_length = 0
    for ds_path in args.original_dataset:
        ds_path = os.path.abspath(ds_path)
        assert os.path.exists(ds_path), f"Not existed file path {ds_path}"
        ds_dict = load_from_disk(dataset_path=ds_path, keep_in_memory=False)
        if isinstance(ds_dict, HFDataset):
            # ds_dict = ds_dict.select(random.sample([i for i in range(len(ds_dict))], int(len(ds_dict)*2.5/8.1)))
            datasets.append(ds_dict)
            dataset_length += len(ds_dict)
        else:
            raise ValueError("Only hf dataset is allowed")
    coordinator.print_on_master("original_dataset loaded")
    coordinator.print_on_master(f"Total size: {dataset_length}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_dataset", nargs="*", default=[])
    parser.add_argument("--augment_dataset", nargs="*", default=[])
    parser.add_argument(
    ...
    parser.add_argument("--beacon_window", type=int, default=1024)
    parser.add_argument("--beacon_stride", type=int, nargs='+', default=[1024])
    parser.add_argument("--beacon_stride_mix", type=str, default='step-random')
    parser.add_argument("--beacon_attn", type=str, default="step-expansion")
    parser.add_argument("--beacon_attend_previous", type=bool, default=True)
    parser.add_argument("--beacon_ratio", type=int, nargs='+', default=[2,4,8,16,32,64,128])
    parser.add_argument("--beacon_ratio_mix", type=str, default="step-random")
    parser.add_argument("--beacon_param", type=str, nargs='+', default=['q', 'k', 'v', 'o'])
    ...
    parser.add_argument("--rope_factor", type=float, default=1.0)
